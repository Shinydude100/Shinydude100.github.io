...
└── Cloudflare Worker (dynamic)   → /mcp (live MCP server with 6 custom tools)
                                  → /api/telemetry (real Cloudflare edge data endpoint)

```

* **Static layer:** Raw HTML, vanilla JavaScript, zero-trust local font hosting. No heavy frameworks, no hydration, no layout shifts.
* **Dynamic layer:** Cloudflare Worker (`remote-mcp-server`) serving a live MCP server at `/mcp` using Streamable HTTP transport, with 6 custom tools for agent consumption. Also serves `/api/telemetry` returning real Cloudflare edge data (colo, city, country, TLS version, CF-RAY, ASN).
...
The site implements 7 machine-readable discovery surfaces, achieving verified Level 5 Agentic Readiness scores (10/10):
...
The live MCP server at `https://daca.me/mcp` exposes 6 tools:
...
| `get_track_record` | Returns track record including DaedalusX64, zero-trust deployments, and infrastructure projects |

## File Structure

* `index.html`: Core entry point. Single-page application with inline JSON-LD (Person + DaedalusX64 founderOf), MCP discovery links, live telemetry data attributes, and terminal-inspired UI.
* `memory-backplane.js`: Client-side matrix background canvas animation logic, separated from HTML for CSP compliance.
* `script.js`: UI interactions, WebMCP integration, and real edge telemetry consumption from `/api/telemetry`.
...
* `schema.jsonld`: Schema.org `Person` structured data with `sameAs` (GitHub, LinkedIn) and `founderOf` (DaedalusX64) for entity disambiguation.
...
| **Worker Route** | `daca.me/api/telemetry*` → `remote-mcp-server` |
...
| Auditor | Score | Date |
| --- | --- | --- |
| **Gemini Spark** | **10.0 / 10** | **2026-08-03** |
| **ChatGPT** | **9.5 / 10** | **2026-08-03** |
